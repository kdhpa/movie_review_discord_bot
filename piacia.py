...
from review_form import MOVIE_FORM, MANGA_FORM, WEBTOON_FORM, SEASON_LABEL, format_season
...
def parse_review_message(content):
    ...
    첫 줄이 '{emoji}제목: {title}' 또는 '{emoji}제목: {title} {N}{시즌/기/부}' 형식
    ...
    for emoji, category in EMOJI_CATEGORY.items():
        ...
        if first_line.startswith(prefix):
            title_with_season = first_line[len(prefix):].strip()
            # "원피스 1기", "브레이킹 배드 시즌2", "나루토 3부" 패턴 매칭
            match = re.match(r'^(.+?)\s+(\d+)(시즌|기|부)$', title_with_season)
            if match:
                return match.group(1), category, int(match.group(2))
            return title_with_season, category, None
...
async def _save_and_send_review(
    ...
    if db.has_review(author_id, title, db_category, season=season):
...
class ReviewForm(discord.ui.Modal, title="한줄평 작성"):
    def __init__(self, db, category, author_id: int, id_name: str, author_name: str, prefetched_info: tuple = None, prefetched_category: str = None, default_season: int = None):
        super().__init__()
        self.db = db
        ...
        season_default = str(default_season) if default_season else ""
        ...
        if prefetched_info:
            ...
            self.add_item(discord.ui.TextInput(label="시즌/기 번호 (선택, 미입력=전체)", style=discord.TextStyle.short, required=False, placeholder="예: 1 (영화는 미입력)", default=season_default))
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 필드 인덱스: [0]=title, [1]=season, [2]=score, [3]=review, [4]=comment
        if self.prefetched_info:
            ...
            score = self.children[2].value  # 별점
        ...
        # 시즌 파싱
        season_str = self.children[1].value.strip()
        if season_str:
            try:
                self.season = int(season_str)
                if self.season < 1:
                    await interaction.response.send_message("❌ 시즌 번호는 1 이상이어야 합니다!", ephemeral=True)
                    return
            except ValueError:
                await interaction.response.send_message("❌ 시즌 번호는 숫자만 입력해주세요!", ephemeral=True)
                return
        else:
            self.season = None
        ...
        if self.prefetched_info:
            ...
            prefetched_cat = self.prefetched_category or 'manga'
            # 영화 카테고리면 시즌 무시
            effective_season = None if prefetched_cat == 'movie' else self.season
            movie_info = {
                'title': title,
                'year': year,
                'director': director,
                'img_url': img_url,
                'category': prefetched_cat,
...
class EditReviewForm(discord.ui.Modal):
    def __init__(self, db, review_data, channel, user_id, display_name, target_message=None):
        # Modal title에 시즌 정보 포함
        season = review_data.get('season')
        season_text = format_season(review_data.get('category'), season)
        title_text = f"리뷰 수정 - {review_data['movie_title']}{season_text}"
...
@discord.app_commands.describe(제목="삭제할 작품 제목", 카테고리="카테고리", 시즌="삭제할 시즌 번호 (미입력 시 전체 리뷰 삭제)")
@discord.app_commands.choices(카테고리=[
    discord.app_commands.Choice(name="영화", value="movie"),
    discord.app_commands.Choice(name="드라마", value="drama"),
    discord.app_commands.Choice(name="애니", value="anime"),
...
async def delete_review_command(interaction: discord.Interaction, 제목: str, 카테고리: str = None, 시즌: int = None):
    await interaction.response.defer(ephemeral=True)

    # 시즌 미지정 시 해당 제목의 리뷰가 여러 건인지 확인
    if 시즌 is None and 카테고리:
        reviews_for_title = bot.db.get_user_reviews_for_title(interaction.user.id, 제목, 카테고리)
        if len(reviews_for_title) > 1:
            review_list = []
            for r in reviews_for_title:
                s = r.get('season')
                season_text = format_season(r['category'], s)
                label = f"전체 리뷰" if s is None else f"{season_text.strip()}"
                review_list.append(f"• {label} (⭐ {r['score']}/5)")
            await interaction.followup.send(
                f"📋 '{제목}'에 대한 리뷰가 {len(reviews_for_title)}개 있습니다:\n"
                + "\n".join(review_list) +
                "\n\n`/리뷰삭제`에서 `시즌` 파라미터를 지정해주세요. (전체 리뷰 삭제는 시즌=0)",
                ephemeral=True
            )
            return

    # 시즌 값 결정: 0은 전체 리뷰(season IS NULL)를 의미
    season_value = None if (시즌 is None or 시즌 == 0) else 시즌

    # 삭제 전 기존 데이터 조회 (로그용 + 메시지 삭제용)
    review = bot.db.get_user_review(interaction.user.id, 제목, 카테고리, season=season_value)
    ...
    deleted = bot.db.delete_review(interaction.user.id, 제목, 카테고리, season=season_value)
...
@discord.app_commands.describe(제목="수정할 작품 제목", 카테고리="카테고리", 시즌="수정할 시즌 번호 (미입력 시 전체 리뷰 수정)")
@discord.app_commands.choices(카테고리=[
    discord.app_commands.Choice(name="영화", value="movie"),
    discord.app_commands.Choice(name="드라마", value="drama"),
    discord.app_commands.Choice(name="애니", value="anime"),
...
async def edit_review_command(interaction: discord.Interaction, 제목: str, 카테고리: str = None, 시즌: int = None):
    # 시즌 미지정 시 해당 제목의 리뷰가 여러 건인지 확인
    if 시즌 is None and 카테고리:
        reviews_for_title = bot.db.get_user_reviews_for_title(interaction.user.id, 제목, 카테고리)
        if len(reviews_for_title) > 1:
            ...
            await interaction.response.send_message(
                f"📋 '{제목}'에 대한 리뷰가 {len(reviews_for_title)}개 있습니다:\n"
                + "\n".join(review_list) +
                "\n\n`/리뷰수정`에서 `시즌` 파라미터를 지정해주세요. (전체 리뷰 수정은 시즌=0)",
                ephemeral=True
            )
            return
