...
- 앨범/곡은 `링크` 옵션으로 Spotify 트랙/앨범 또는 YouTube Music 곡 링크를 넣으면 제목/아티스트/커버를 채운 입력창을 바로 엽니다. `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET`이 있으면 Spotify Web API를 먼저 쓰고, `YOUTUBE_API_KEY`가 있으면 YouTube Data API를 먼저 씁니다. API가 없거나 실패하면 페이지 메타/oEmbed를 마지막 fallback으로 사용합니다.
- DB에 자동 저장됨
